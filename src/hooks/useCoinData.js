    ...
    const fetchCoinData = async () => {
      try {
        const marketDataResponse = await axios.get(
          `https://api.coingecko.com/api/v3/coins/markets?x_cg_demo_api_key=CG-agp1scJQqR3JsindWUSS9ZF4&vs_currency=usd&ids=${coinId}`
        );

        const detailedDataResponse = await axios.get(
          `https://api.coingecko.com/api/v3/coins/${coinId}?x_cg_demo_api_key=CG-agp1scJQqR3JsindWUSS9ZF4`
        );
